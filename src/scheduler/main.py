...
class Scheduler:
    ...
    def send_request_to_service_life_cyle(self,username,application_id,service_name,service_instance_id,type_):
        response = {"username":username,"applicationname":application_id,"servicename":service_name,"serviceId":self.main_service_id_dict[service_instance_id]}        # if type_=="start":
        if type_=="start":
...
def schedule_service():
    ...
    if(content["action"]=="Stop"):
        id_ = content["username"]+"_"+content["applicationname"]+"_"+content["service_name"]
        response = {"userId":content["username"],"applicationName":content["applicationname"],"servicName":content["service_name"],"serviceId":id_}
        # res = requests.post('http://'+service_life_cycle_ip+':'+str(service_life_cycle_port)+'/servicelcm/service/stop', json=response)
